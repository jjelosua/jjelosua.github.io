...
Anyhow these were my first elections inside a newsroom and it was thrilling!! It was like being inside a movie for a weekend where everybody was waiting for the initial results to rush to their computers to write articles and analysis....but we needed data and I can tell you it was not easy.
...
![Calendar](/images/2015_paso_caba/calendar.png){.post_left width=48%}In Argentina voting is a compulsory right for the elegible population so Argentinians will be voting many times during this year. One of the first things that stroke me as a foreigner is that PASO elections are not withheld inside the political party affiliates but rather extended to the whole census. In Spain Primary elections only affect the affiliates and thus are rather transparent to the overall population. 
...
At La Nación we wanted to make a news app that would allow readers to perform a realtime analysis of the results of the elections. We also wanted to get as detailed as possible given that we had information, we wanted to show the results by polling station.
...
That way we could move forward in defining the final vizualizations and the different reader interaction capabilities and drilldown analysis.
...
We did not have data yet to test the process so we developed a adhoc simulation script that would randomly produce voting results distributions. My wife to the rescue...[Marta](https://twitter.com/marta_alonso_f) helped us develop that results simulation and I believe that was a key decision in the overall success of the project since it allowed us to start testing the backend well ahead of the API availability.

We finally were given an endpoint to test the government API by friday 24th at 18h...fun weekend ahead. 
...
We developed a logging strategy that was published each time the process tried to run (each minute) and that we made accesible through HTTP.

We used [Postman](https://chrome.google.com/webstore/detail/postman-rest-client-packa/fhbjgbiflinjbdggehcddcbncdddomop?hl=en){:target="_blank"} a chrome app that lets you define and save collections of HTTP requests:
...
Being a freelance developer that used to work with NGOs in Spain to increase transparency and the use of Open Data I had not being exposed to the traffic that the homepage of La Nación receives on an election day. So It was really interesting for me to learn from the infrastructure and architecture team the tips && tricks that we needed to take into account in order to be succesfull: HTTP caches, Content Delivery Networks configuration and static files versioning...All of that new to me but really important in terms of performance and bug fixing on the fly.
...
After sometime were the API was not responding we were able to get a full response to our backend and transform it to the frontend expected format...that was at 22:30 Sunday night...we even published the initial results before the official government page that was suffering from availability issues.
...
Thanks to that we could delivered a map that showd the results for each polling station the same day we got the official results from the Buenos Aires Government.
...
We used [cartodbjs API](http://docs.cartodb.com/cartodb-platform/cartodb-js.html){:target="_blank"} [requirejs](http://requirejs.org/){:target="_blank"} in order to keep the javascript development modular. 
...
I am really grateful for being able to have lived the election experience inside a newsroom. We have worked hard as a team and the overall results were really positive...really proud of the team and the effort.

Thanks to the rest of the team [Cristian Bertelegni](https://twitter.com/cbertelegni), [Gaston de la Llana](https://twitter.com/gasgas83) and [Pablo Loscri](https://twitter.com/ploscri) for keeping such a nice working atmosphere. I would also like to send special thanks to Flor Fernández, Mariana Trigo and my wife Marta for helping us keep calmed when everything seemed impossible.   
